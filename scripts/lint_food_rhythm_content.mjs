...
const foodRhythmPath = path.join(root, "content", "scheduled", "food-rhythm-articles.json");
...
if (!fs.existsSync(foodRhythmPath)) {
  console.log("No Food Rhythm article file present.");
  process.exit(0);
}

const articles = JSON.parse(fs.readFileSync(foodRhythmPath, "utf8"));
if (!Array.isArray(articles)) {
  errors.push("food-rhythm-articles.json must be an array.");
} else if (articles.length !== 50) {
  errors.push(`food-rhythm-articles.json must include exactly 50 articles; found ${articles.length}.`);
}

if (Array.isArray(articles)) {
  for (const article of articles) {
    const label = `food-rhythm:${article.slug || article.title}`;
...
console.log("Food Rhythm content lint passed.");
